...
export function Workspace<TTab extends string = string>({
	...
	const [internalLayout, setInternalLayout] =
		useState<LayoutNode<TTab>>(defaultLayout);
	...
	useEffect(() => {
		...
		const onPointerMove = (event: globalThis.PointerEvent) => {
			...
			const deltaRatio =
				(client - resizeState.startClient) / resizeState.extentPx;
			...
			const minRatio = Math.min(
				pairTotal / 2,
				minPaneSize / resizeState.extentPx,
			);
			const first = clamp(
				firstStart + deltaRatio,
				minRatio,
				pairTotal - minRatio,
			);
			...
			updateLayout((current) =>
				setSplitSizesAtPath(current, resizeState.path, sizes),
			);
		};
	...
	const workspaceClass = cx(
		styles.workspace,
		resizeState !== null && styles.resizing,
		className,
	);
	...
	function renderSplit(node: SplitNode<TTab>, path: number[]): ReactNode {
		...
		const splitClass = cx(
			styles.split,
			node.dir === "row" ? styles.splitRow : styles.splitCol,
		);

		return (
			<div
				className={splitClass}
				data-ui="workspace-split"
				data-direction={node.dir}
			>
				{node.children.map((child, index) => (
					<Fragment
						key={`${path.join(".")}.${index}.${child.type === "leaf" ? child.id : child.dir}`}
					>
						<div
	...
	function renderLeaf(leaf: LeafNode<TTab>): ReactNode {
		...
		return (
			...
			>
				...
				>
					{leaf.tabs.map((tab) => (
						<button
							...
							className={cx(
								styles.tab,
								leaf.activeTab === tab && styles.tabActive,
							)}
							data-ui="pane-tab"
							data-tab-value={tab}
							draggable
							key={tab}
							onClick={() =>
								updateLayout((current) => setActiveTab(current, leaf.id, tab))
							}
							onDragEnd={() => setDragState(null)}
							onDragStart={(event) => startTabDrag(event, leaf.id, tab)}
							onKeyDown={(event) => handleLeafTabKeyDown(event, leaf, tab)}
							role="tab"
							tabIndex={leaf.activeTab === tab ? 0 : -1}
							title={String(tab)}
							type="button"
						>
					...
					{toolbar !== undefined && (
						<div className={styles.leafToolbar} data-ui="pane-toolbar">
							{toolbar}
						</div>
					)}
				</div>
				...
				{activeDrop !== null && (
					<div
						className={cx(styles.dropIndicator, dropEdgeClass(activeDrop))}
						aria-hidden="true"
					/>
				)}
			</section>
	...
	function handleLeafTabKeyDown(
		event: KeyboardEvent<HTMLButtonElement>,
		leaf: LeafNode<TTab>,
		tab: TTab,
	) {
		const currentIndex = leaf.tabs.indexOf(tab);
		let nextIndex = currentIndex;

		if (event.key === "ArrowRight") {
			nextIndex = (currentIndex + 1) % leaf.tabs.length;
		} else if (event.key === "ArrowLeft") {
			nextIndex = (currentIndex - 1 + leaf.tabs.length) % leaf.tabs.length;
		} else if (event.key === "Home") {
			nextIndex = 0;
		} else if (event.key === "End") {
			nextIndex = leaf.tabs.length - 1;
		} else {
			return;
		}

		const nextTab = leaf.tabs[nextIndex];
		if (nextTab === undefined) return;

		event.preventDefault();
		updateLayout((current) => setActiveTab(current, leaf.id, nextTab));
		focusTabButton(event.currentTarget, nextIndex);
	}
	...
	function startTabStripDrag(
		...
		if (
			...
			target.closest('[data-ui="pane-tab"]') !== null
		)
			return;
		if (
			target instanceof HTMLElement &&
			target.closest('[data-ui="pane-toolbar"]') !== null
		) {
			event.preventDefault();
			return;
		}
...
function focusTabButton(trigger: HTMLButtonElement, index: number) {
